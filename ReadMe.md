...
1️⃣ User uploads image using Streamlit UI  
2️⃣ Image (bytes format) is sent to FastAPI `/detect` API  
3️⃣ Backend converts bytes → cv2 BGR → YOLO Blob  
4️⃣ YOLOv3 runs detections using COCO dataset trained weights  
5️⃣ Bounding boxes and labels are drawn using Pillow  
6️⃣ Annotated image is returned (Base64) & displayed to the user  
7️⃣ Download option available for both JSON & output image
